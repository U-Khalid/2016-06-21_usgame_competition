...
/**
 * Servlet implementation class toloadMechanism
 */
@WebServlet("/uploadMechanism")
public class uploadMechanism extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {
					...
					{
						if(!value.equals("")){
							info.settMechanism_ID(Integer.parseInt(value));
						}
					}
					if(name.equals("tMechanism_Name"))
					{
						info.settMechanism_Name(value);
					}
					if(name.equals("tMechanism_Address"))
					{
						info.settMechanism_Address(value);
					}
					if(name.equals("tMechanism_Phone"))
					{
						info.settMechanism_Phone(value);
					}
					if(name.equals("tMechanism_Detail"))
					{
						info.settMechanism_Detail(value);
					}
					...
					if(fileformName.equals("tMechanism_PictureUrl")){
						info.settMechanism_PictureUrl(filename);
					}
		...
		result = trainservice.addMechanism(info);
		String state;
		if(result) {
			state="培训机构创建成功";
			if(!message.equals("")){
